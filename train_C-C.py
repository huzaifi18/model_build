...
def main():
    ...
    for j, (train_idx, val_idx) in enumerate(folds):
        ...
        trainX, trainY, SS_tr = utils.extract_VIT_capacity(train_x_data, train_y_data, 5, 1, 10,
                                                           extract_all=False,
                                                           extract_c_only=True)
        valX, valY, SS_val = utils.extract_VIT_capacity(val_x_data, val_y_data, 5, 1, 10,
                                                        extract_all=False,
                                                        extract_c_only=True)
        testX, testY, SS_tt = utils.extract_VIT_capacity(test_x_data, test_y_data, 5, 1, 10,
                                                         extract_all=False,
                                                         extract_c_only=True)
        print('Input shape: {}'.format(trainX.shape))

        model = Sequential()
        model.add(LSTM(32, input_shape=(5, 1), activation='tanh', return_sequences=True))
        ...
        inv_valY = SS_val[0][0].inverse_transform(valY)
        inv_valPredict = SS_val[0][0].inverse_transform(valPredict)

        inv_testY = SS_tt[0][0].inverse_transform(testY)
        inv_testPredict = SS_tt[0][0].inverse_transform(testPredict)
